...
patches/6b4/msromdumper-6b4.gen: patches/ucode-exit.txt patches/msromdumper-ucode.txt
	cat patches/ucode-exit.txt > $(@D)/msromdumper-6b4.gen
	sed -e 's/ROMDUMPER_DONE/UROM_26CA/g' < patches/msromdumper-ucode.txt >> $(@D)/msromdumper-6b4.gen

patches/6d6/msromdumper-6d6.gen: patches/ucode-exit-pm.txt patches/msromdumper-ucode.txt
	cat patches/ucode-exit-pm.txt > $(@D)/msromdumper-6d6.gen
	sed -e 's/ROMDUMPER_DONE/UROM_1B41/g' < patches/msromdumper-ucode.txt >> $(@D)/msromdumper-6d6.gen
...
patches/6d6/utool-6d6.gen: patches/ucode-exit-pm.txt patches/utool-ucode.txt
	cat patches/ucode-exit-pm.txt > $(@D)/utool-6d6.gen
	sed -e 's/UROM_CPUID_DONE/UROM_332C/g' -e 's/UROM_CPUID_OTHER/UROM_1B3E/g' < patches/utool-ucode.txt >> $(@D)/utool-6d6.gen
